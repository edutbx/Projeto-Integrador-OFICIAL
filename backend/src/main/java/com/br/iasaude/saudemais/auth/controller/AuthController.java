...
public class AuthController {
    ...
    public ResponseEntity<?> register(@Valid @RequestBody RegisterRequest req){
        ...
        return ResponseEntity.ok(new AuthResponse(token, u.getNome(), u.getSobrenome(), u.getCpf(), u.getRg(), u.getDataNascimento(), u.getSexo(), u.getCrm(),
                u.getEspecializacao(), u.getIdGestor(), u.getEmail(), u.getCep(), u.getLogradouro(), u.getNumero(), u.getComplemento(), u.getCidade(), u.getEstado()));
    }
    ...
    response.addCookie(cookie);
        return ResponseEntity.ok(new AuthResponse(token, user.getNome(), user.getSobrenome(), user.getCpf(), user.getRg(), user.getDataNascimento(), user.getSexo(), user.getCrm(),
                user.getEspecializacao(), user.getIdGestor(), user.getEmail(), user.getCep(), user.getLogradouro(), user.getNumero(), user.getComplemento(), user.getCidade(), user.getEstado()));
    }
    ...
    /**
     * Endpoint de verificação de sessão chamado pelo frontend.
     * O JwtAuthenticationFilter valida o token antes de chegar aqui.
     * 200 = token válido | 401 = não autenticado (tratado pelo Spring Security).
     */
    @GetMapping("/me")
    public ResponseEntity<?> me(Authentication authentication) {
        ...
        var userOpt = userRepository.findByCrm(crm);
        if (userOpt.isEmpty()) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("error", "Usuario nao encontrado"));
        }
        Usuario u = userOpt.get();
        return ResponseEntity.ok(Map.of("nome", u.getNome(), "crm", u.getCrm()));
    }
